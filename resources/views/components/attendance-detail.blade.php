    ...
    <div class="container max-w-[1400px] mx-auto px-8 py-4 flex flex-col items-center min-h-[calc(100vh-80px)]">
        ...
        @endif
            ...
            <form action="{{ route('attendance.update', ['id' => $attendance->id]) }}" method="POST">
                @csrf
                @php
                    $isPending = isset($attendanceCorrect) && $attendanceCorrect !== null;
                    ...
                    @if (Auth::user()->can('admin'))
                        ...
                        @else
                            <button type="button" disabled
                                class="bg-[#8B8B8B] text-white px-12 py-3 text-xl font-bold rounded-md cursor-not-allowed">承認済み</button>
                        @endif
                    @else
                        @if ($isPending)
                    ...
                    fetch(url, {
                        ...
                        .then(response => {
                            if (response.ok) {
                                ...
                                messageBox.className = "max-w-[900px] bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded my-2 mx-auto text-center";
                                messageBox.innerText = "承認しました";

                                const header = document.querySelector('h1');
                                header.parentNode.insertBefore(messageBox, header);

                                this.outerHTML = '<button type="button" disabled class="bg-[#8B8B8B] text-white px-12 py-3 text-xl font-bold rounded-md cursor-not-allowed">承認済み</button>';
